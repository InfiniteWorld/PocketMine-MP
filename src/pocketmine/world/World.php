...
class World implements ChunkManager, Metadatable{
	...
	public static function blockHash(int $x, int $y, int $z){
		...
		return PHP_INT_SIZE === 8 ? (($x & 0xFFFFFFF) << 36) | (($y & World::Y_MASK) << 28) | ($z & 0xFFFFFFF) : $x . ":" . $y . ":" . $z;
	}
	...
	public function __construct(Server $server, string $name, WritableWorldProvider $provider){
		...
		$this->time = (int) $this->provider->getWorldData()->getTime();
	...
	public function sendTime(Player ...$targets){
		...
		$pk->time = (int) $this->time & 0xffffffff; //avoid overflowing the field, since the packet uses an int32
	...
	public function save(bool $force = false) : bool{
		...
		$this->provider->getWorldData()->setTime((int) $this->time);
		$this->saveChunks();
